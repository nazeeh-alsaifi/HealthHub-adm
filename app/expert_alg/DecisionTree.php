...
class DecisionTree extends Tree
{
    ...
    public function predictUsingArray($array)
    {

        $row = $array;
        $result = $this->predict($this->root, $row);
    ...
    private function find_root($parent_node, $branch_name, $training_data)
    {
        ...
        if (count($header) == 0) {
            foreach ($p as $value => $count) {
                $max_count = 0;
                $max_val = "";
                if (empty($max_val) || ($count > $max_count)) {
                    $max_val = $value;
                    $max_count = $count;
                }
            }
            $parent_node->namedBranches[$branch_name] = new Node(strtoupper($max_val));
            return $header;
        }

        $sum = 0;
        foreach (array_values($header) as $val) {
            $sum += $val;
        }
        if ($sum == 0) {
            foreach ($p as $value => $count) {
        ...
        if ($winning_attribute != 'none')
            $training_data['header'][$winning_attribute] = 0;
        if ($parent_node->value != 'Root') {
        ...
        // if ($p == "none" && count($header) == 0){ return
        foreach ($p as $value => $count) {
            ...
            $training_data['header'] = $this->find_root($parent_node, $value, $subset);
        }
        return $training_data['header'];
    }
